...
<header class="flex justify-between items-center mb-10">
    <div>
        <h1 class="text-3xl font-black italic text-indigo-900 border-b-4 border-indigo-900 pb-1 inline-block tracking-tight">Kelola Event</h1>
        <p class="text-slate-500 font-medium mt-2">Buat dan atur acara seru Anda di sini.</p>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        {{-- Logika pengaman: Jika data lama masih pakai 'posters/', gunakan fallback storage, jika data baru langsung panggil path-nya --}}
                        @if(Str::startsWith($event->poster_path, 'images/'))
                            <img src="{{ asset($event->poster_path) }}" class="w-16 h-20 rounded-xl object-cover shadow-sm">
                        @else
